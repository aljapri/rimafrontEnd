...
const initialFormState = {
  ...
  email: "",
  password: "",
  roleName: "Student", // fixed roleName included
  specialization: "", // add specialization if backend needs it
};
...
export default function CreateStudent() {
  ...
  const validate = () => {
    ...
    if (!formData.password) {
      ...
      !/(?=.*[@$!%*?&#^+=._-])/.test(formData.password) // at least one symbol
    ) {
      newErrors.password =
        "Password must contain lowercase, uppercase, number, and symbol";
    }

    // Optional: if you want specialization required
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || "Email already used");
      }
  ...
  return (
    <div className="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md">
      <h1 className="text-3xl font-bold mb-6 text-gray-900">Create Student</h1>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        {/* Uncomment if specialization needed */}
